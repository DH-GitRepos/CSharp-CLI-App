...
{
    public class Command_Menu_TakeQuantityFromItem : UI_Command<Command_DTO>
    {
        ...
        public Command_Menu_TakeQuantityFromItem()
        {
        ...
        public Command_DTO Start_Command_Process()
        {
            try
            {
                ...
                ValidateData["quantityToRemove"] = quantityToRemove;
                ...
                if (validator.Status == "OK")
                {
                    ...
                    ExecuteData["quantityToRemove"] = quantityToRemove;
                    ExecuteData["find_item_dto"] = find_item_dto;

                    Command_DTO execute_return = Execute_Command_Process();
                    Item itemRemove = (Item)execute_return.ReturnItems["item"];

                    TransactionLogEntry transaction = new TransactionLogEntry("Quantity Removed", itemRemove.ID, itemRemove.Name, -1, quantityToRemove, employeeName, DateTime.Now);
        ...
        {
            int quantity = (int)ExecuteData["quantityToRemove"];
            ...
            {
                processedItemDTO = DB_Facade.Item_Update_Remove(find_item_response.ItemEntity, quantity);

                returnResult.Add(string.Format(
                    "\n{0} items have been removed from Item ID: {1} on {2}",
                    quantity,
            ...
            else
            {
                ...
                {
                    status = "ERROR";
                    message = "Cannot remove a quantity less than 0.";
        ...
        public Command_DTO Command_Process_Validator()
        {
            ...
            int quantityToRemove = (int)ValidateData["quantityToRemove"];
            ...
            try
            {
                Item tempItem = new Item(999, "Valid item", quantityToRemove, DateTime.Now);
            }
            catch (Exception e)
            {
                status = "ERROR";
            ...
            if (status == "OK")
            {
                ...
                message = $"\n{quantityToRemove} {item.Name}(s) removed from stock by {employeeName}";
            }
